...
<template id="hylightbox-template">
    <div class="hylightbox-wrapper">
        ...
        <div class="hylightbox-download-container hylb-configurable-ui">
            <button class="hylightbox-btn hylb-open-newtab-btn" title="在新标签页打开">↗</button>
        </div>
        ...
        <div class="hylb-page-indicator hylb-configurable-ui" title="点击查看快捷键指南">1/1</div>
...
<style>
    ...
    .hylightbox-btn:hover {
        opacity: 0.6;
    }

    /* UI 容器通用样式 */
    .hylightbox-control-bar,
    .hylightbox-help-container,
    .hylightbox-close-container,
    .hylightbox-rotate-container,
    .hylightbox-download-container,
    .hylb-page-indicator {
        position: absolute;
    ...
    .hylightbox-help-container.visible {
    ...
    .hylightbox-help-tooltip.visible {
        display: block;
    }
    .hylb-help-title {
        color: #ffffff !important;
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const helpContainer = document.querySelector(".hylightbox-help-container");
        ...
        function openImageInNewTab() {
            if (state.galleryImages.length === 0 || state.currentIndex === -1) return;
            const img = state.galleryImages[state.currentIndex];
            const src = img.getAttribute("data-src") || img.src;
        ...
        document.querySelector(".hylb-prev-btn").onclick = (e) => {
            e.stopPropagation();
            navigate(-1);
        };
        document.querySelector(".hylb-next-btn").onclick = (e) => {
            e.stopPropagation();
            navigate(1);
        };
        document.querySelector(".hylb-zoom-in-btn").onclick = (e) => {
            e.stopPropagation();
            handleZoom("in");
        };
        document.querySelector(".hylb-zoom-out-btn").onclick = (e) => {
            e.stopPropagation();
            handleZoom("out");
        };
        document.querySelector(".hylb-rotate-btn").onclick = (e) => {
            e.stopPropagation();
            state.transform.rotation += 45;
            applyTransform();
        };
        document.querySelector(".hylb-rotate-counter-btn").onclick = (e) => {
            e.stopPropagation();
            state.transform.rotation -= 45;
            applyTransform();
        };
        document.querySelector(".hylb-close-x-btn").onclick = (e) => {
            e.stopPropagation();
            closeLightbox();
        };
        document.querySelector(".hylb-open-newtab-btn").onclick = (e) => {
            e.stopPropagation();
            openImageInNewTab();
        };
        document.querySelector(".hylb-page-indicator").onclick = (e) => {
            e.stopPropagation();
        ...
        wrapper.onwheel = (e) => {
            e.preventDefault();
            state.transform.scale *= e.deltaY < 0 ? 1.04 : 0.96;
            applyTransform();
        };
